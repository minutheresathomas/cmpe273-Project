...
public class UserPollResource {
									...
									@Valid Poll request) throws Exception
	{
		...
		usersRepository.updateCreatedPoll(user_id, poll.getId());
	...
	@Path("/createUser")
	@Timed(name= "create-poll")
	public Response createNewUser(@Valid User request) throws Exception
	{
		HashMap<String, Object> myMap = new HashMap<String, Object>();
		String data = "{Success}";
		try {
		User user = usersRepository.saveUser(request);
		System.out.println("email id is : "+user.getUser_id());
//		String data = "{Success}";
		List<PollDto> userLink = new ArrayList<PollDto>();
		userLink.add(new PollDto("create-poll", "/"+user.getUser_id()+"/createPoll" , "POST"));
		myMap.put("userOperations", userLink);
		} catch(Exception e) {
			e.printStackTrace();
		}
		return Response.status(200).entity(myMap).build();
	}
	...
	@Path("/{user_id}")
	public Response authenticateUser(@PathParam("user_id") String user_id) 
	{
		String success = usersRepository.authenticateUser(user_id);
...
//		}
	...
	public Response participatePoll(@PathParam("user_id") String user_id,
							@PathParam("id") String id, 
							@QueryParam("option") String option)
	{
		pollsRepository.setCountForOption(id, option);
		boolean val = usersRepository.updateUserPolls(user_id, id);
		Poll pollById = pollsRepository.getPollById(id);
		if(val) {
//			HashMap<String, Object> myMap = new HashMap<String, Object>();
//			myMap.put("Poll", pollById);
	...
	public HashMap<String, Object> searchPoll(@PathParam("user_id") String user_id, 
					@QueryParam("question") String que)
	{
